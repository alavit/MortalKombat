import { generateRandomNum, getCurrentTime } from './utils.js';
...
function generateLogs(type, player1, player2, changeValue, hp, resultObj) {
    let fullText = '';
    ...
    switch (type) {
      case 'start':
        ...
        break;
      case 'end':
        const {playerWins, playerLoses} = resultObj;
        fullText = `${logs[type][generateRandomNum(logs[type].length - 1)].replace('[playerWins]', playerWins).replace('[playerLose]', playerLoses)}`;
        break;
      case 'hit':
